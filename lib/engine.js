...
const ms = (start) => Math.round((now() - start) * 100) / 100;

export function decide(applicant, config) {
  const t0 = now();
  const trace = [];
  const log = (step, detail) =>
    trace.push({ step, detail, elapsedMs: ms(t0) });

  const amountNum = Number(applicant.loanAmount) || 0;
  log("intake", `Application received: ${labelDestination(applicant.destination)}, ${labelDegree(applicant.degreeLevel)} in ${labelField(applicant.fieldOfStudy)}, amount $${amountNum.toLocaleString("en-US")}`);
  ...
  for (const rule of config.eligibilityRules) {
    ...
    if (!result.pass) failures.push({ ruleId: rule.id, rule: rule.name, reason: result.reason });
  }

  if (failures.length > 0) {
    const n = failures.length;
    log("decision", `Declined: failed ${n} eligibility rule${n === 1 ? "" : "s"}`);
    return {
      decision: "DECLINED",
      stage: "eligibility",
      reasons: failures,
  ...
  // ---- Stage 2: scorecard ----
  // The score is rounded once, here, and that rounded value is what the
  // thresholds, pricing and the response all use. Comparing the raw value
  // while showing the rounded one would let a displayed 65.0 go to review.
  const { score: rawScore, breakdown } = scoreApplicant(applicant, config.scorecard);
  ...
  log("decision", `Score ${score.toFixed(1)} against approve at ${approveAt} and review at ${reviewAt}: ${LABEL[decision]}`);

  // ---- Stage 4: pricing ----
  let pricing = null;
  if (decision === "APPROVED") {
    // Highest band whose floor the score clears. If the approve threshold was
    // set below every band floor, fall back to the lowest band rather than
    // leaving an approved loan unpriced.
    const byFloor = [...config.pricingTiers].sort((a, b) => b.minScore - a.minScore);
    const tier = byFloor.find((t) => score >= t.minScore) ?? byFloor.at(-1);
    pricing = { band: tier.band, apr: tier.apr, label: tier.label };
  ...
  if (decision === "APPROVED") {
    ...
    log("routing", partner ? `Routed to ${partner.name}` : "No partner covers this destination; flagged for operations");
    if (!partner) {
      decision = "REVIEW";
      unroutable = true;
      log("decision", "Approved on score but no partner can fund it: Sent to review");
    }
  }

  const totalMs = ms(t0);
  log("complete", `Decision rendered in ${totalMs < 1 ? "under 1" : totalMs} ms`);
...
const LABEL = { APPROVED: "Approved", REVIEW: "Sent to review", DECLINED: "Declined" };

function labelDestination(c) {
  return { US: "the United States", CA: "Canada", GB: "the United Kingdom" }[c] || c;
}
function labelDegree(d) {
  return { masters: "Master's", mba: "MBA", phd: "PhD", undergraduate: "Undergraduate" }[d] || "Graduate";
}
...
function runRule(rule, a) {
  ...
  switch (rule.id) {
    case "amount-range": {
      ...
      if (amount < minAmount) return { pass: false, reason: `Amount below floor of $${minAmount.toLocaleString("en-US")}` };
      if (amount > maxAmount) return { pass: false, reason: `Amount above ceiling of $${maxAmount.toLocaleString("en-US")}` };
      return { pass: true };
    }
    case "supported-destination":
      return rule.params.destinations.includes(a.destination)
        ? { pass: true }
        : { pass: false, reason: `No active lending partner for ${labelDestination(a.destination)}` };
