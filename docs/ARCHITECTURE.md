...
- Update the decision as the visitor changes any input, with no submit step.
...
| Fast | The engine does a handful of comparisons and six multiplications. Once warm, a call finishes in well under a millisecond on a laptop. The first call on a cold instance is slower. That is what makes deciding on every change practical. |
| No secrets | Nothing in the app needs an API key, a credential or an environment variable. |
| No database | The default policy ships in the code. An edited policy lives in the visitor's browser. The server keeps only rate-limit counters. |
...
| Accessible | Semantic HTML, labeled fields, keyboard operable with visible focus, a skip link, focus never hidden under the sticky header (scroll padding), errors next to the field they belong to, sliders that announce their values, a live status region for results, outcome icons so no status depends on color alone, reduced motion, dark and light themes. Checked with axe in CI (section 8). |
...
|  Apply (live)          Rules console          Policy tests         |
|  Controls, Pipeline,   impact line                                 |
|  DecisionView                                                      |
|       |                     |                     |                |
...
The browser decides first and the server confirms. [ADR 0004](adr/0004-decide-in-browser-confirm-on-server.md) records why.

1. The page opens on the Borderline sample. The visitor picks a sample or changes an input.
2. The browser parses the inputs with `applicantSchema`. If a field fails, its message appears next to it, the last valid result stays on screen with a note, and nothing is sent.
3. If the inputs are valid, the browser runs `decide()` and `suggestChange()` from the same modules the server uses, against the active policy, and renders the result straight away: the pipeline stepper, score and deltas, factor breakdown, offer, reasons, suggestion and trace.
4. After 500 ms with no further change, the browser sends `POST /api/decide` with `{ applicant }`, plus the edited policy as `config` if there is one. With the default policy, `config` is left out and the server uses its own copy. A newer change cancels a pending request.
5. The server checks the method and applies the rate limit. Only then does it look at the body: it rejects a content type other than JSON, reads the body itself with a 32 KB cap (Next's body parser is switched off so this order holds), parses the JSON, and validates it with `decideRequestSchema`.
6. The server calls `decide(applicant, policy)` and then `suggestChange(applicant, policy, result)`. The suggestion reuses the result it was given and only re-runs the engine to search for a loan amount that would be approved. Because a smaller loan never lowers the score, the approving amounts form one range starting at the floor, so a binary search over round thousands, from the floor (the larger of $1,000 and the enabled minimum amount) up to just below the request, finds the largest one in a logarithmic number of engine runs.
7. The browser compares the server's decision, stage, score, band, factor points and failed rules with its own. When they match, the page says "Confirmed by the decision API in N ms". A mismatch is reported as an error. If the API is unreachable or rate limited, the page says it is showing the browser's result. The screen-reader status line announces the outcome once this check settles, so it does not chatter while a slider moves.
...
2. The console builds the next policy and parses it with `configSchema`. If it fails (a floor above the ceiling, a better band priced above a worse one, and so on), the field says why and "Not saved; the rule still uses" the current value.
...
5. The Apply page decides with the edited policy from then on and sends it with each confirmation request. Other open tabs pick up the change through the browser's `storage` event.
...
| `loanAmount` | Whole dollars, 1,000 to 500,000. Amounts outside the program limits are valid input and are declined by the amount rule, with a reason. |
| `householdIncome` | Whole dollars: 0, or 1,000 to 10,000,000 (1 to 999 is refused as a likely typo) |
| `creditHistory` | `established`, `thin` or `none` |
| `workExpYears` | 0 to 40, at most one decimal place |
| `admitConfirmed` | Boolean |

Numeric fields also accept numeric strings, because that is what HTML form fields produce. Strings are trimmed, a blank string counts as missing (not zero), and only plain decimals are accepted, so hex, exponents and thousands separators are rejected. Every refusal comes with a written message, never a raw library default.

`config` is optional. When present, it must pass `configSchema`: 1 to 20 eligibility rules drawn from the six known rule ids, each id at most once, each with the parameters that rule needs; loan limits in whole dollars from 1,000 to 500,000; scorecard weights between 0 and 100 that add up to 100; thresholds and band floors with at most one decimal (scores carry one); field scores and degree multipliers between 0 and 1; review threshold at or below the approve threshold; 1 to 8 pricing bands with unique names, distinct minimum scores and APRs from 0 to 40 with at most two decimals, where the APR never rises as the minimum score rises; up to 20 partners. String and list lengths are capped throughout.
...
- `Retry-After` on a 429 and `Allow: POST` on a 405, as listed above.

## 5. Trust boundaries

The browser is untrusted, and so is anything it sends, including the policy. Its own live decision is a preview for the visitor; the server never accepts a decision from the client, only the inputs, and decides again.
...
- **The pages are locked down.** `next.config.js` sends a Content Security Policy on every page: `default-src 'self'`, `script-src 'self'`, `style-src 'self' 'unsafe-inline'` (needed for the React style props on the score bars), fonts self-hosted, `object-src 'none'` and `frame-ancestors 'none'`. It also sends `nosniff`, a strict referrer policy, `X-Frame-Options: DENY` and a `Permissions-Policy` that turns off camera, microphone and location. The theme script, `public/theme.js`, is a same-origin file, so `script-src 'self'` allows it with no inline-script exception. The `X-Powered-By` header is removed. Dependencies are pinned (Next 15.5.26, with a `postcss` override) and `npm audit` reports no known vulnerabilities.

## 6. One engine, three uses

`lib/engine.js` has no imports and no side effects. The browser imports it to decide live as the visitor edits and to run the golden applicants for the impact preview and the policy tests page. The server imports the same file to make the decision that confirms it. Because it is the same function with the same inputs, the live result, a simulated outcome in the rules console and the API's answer are identical for the same applicant and policy. There is no second implementation that could drift, and the confirmation step checks that on every settled change.

The same idea applies to validation. `lib/validate.js` runs on the Apply page, in the rules console, when a saved policy is loaded from storage, and in the API. A request the UI would refuse, the API refuses too.

## 7. State and persistence

- Two things persist, both in `localStorage`: the edited policy under `ds-config`, and the theme under `ds-theme`. `public/theme.js` reads the theme before first paint, so the page never flashes the wrong one; dark is the default.
...
The tests are shaped like a pyramid, with the cheapest and most numerous checks at the bottom. `npx vitest run` runs 9 files and 771 tests, all passing.
...
2. **Properties (unit), `tests/invariants.test.js`.** 600 applicants from a fixed seed (`tests/helpers/random.js`), checked against invariants that must hold for every input: determinism and no mutation of inputs; a score between 0 and 100 that equals the sum of its factors; monotonicity (more income, better credit, a better school or a smaller loan never lowers the score); switching an eligibility rule off never worsens an outcome; an approval always has a price and a partner and a knockout names its rule; a higher score never gets a higher rate; every enabled rule appears once in the trace. A regression test pins the fix for an old bug where a raw 64.96 was reported as 65 but compared unrounded and sent to review.
3. **Counterfactual (unit), `tests/counterfactual.test.js` and `tests/counterfactual-search.test.js`.** Every suggestion for the golden set and 300 random applicants is confirmed by re-running the engine. The search tests use a $1,000 to $1,000,000 range, compare the binary search with a linear scan, and count engine runs to prove it stays logarithmic.
4. **Input bounds (unit and integration), `tests/bounds.test.js`.** Every applicant field and every editable policy field at and past its limits, with implausible values, wrong types and oversized strings. Each case is checked against the schema (the exact message, or the parsed value) and through the API handler (a 400 at the right path with that message, never a 500, never a 200 for a bad value).
5. **Supporting units.** `tests/validate.test.js` (schemas and messages), `tests/storage.test.js` (load, save, reset, version mismatch, blocked storage) and `tests/impact.test.js` (known rule edits move exactly the expected golden cases).
6. **API contract and fuzz (integration), `tests/api.test.js`.** The handler is called with mock request and response objects, including a stream path that feeds the body through a `Readable` so the handler's own reader runs. It covers every documented status: 405; 415; 400 for invalid JSON and for invalid applicants and policies, each at its exact issue path; 413 for a declared, streamed or pre-read body over 32 KB (exactly 32,768 bytes still passes); 429 on the 61st request, with invalid requests counted. 600 random malformed bodies must all get a 200 or 400 with no stack trace, and a spoofed `X-Forwarded-For` must not escape the limit.
7. **Accessibility and layout (end to end), `tests/e2e/a11y.mjs`.** Playwright drives Chromium against a running server at four widths (320, 390, 768, 1366) in both themes. It runs axe with the WCAG 2.0, 2.1 and 2.2 A and AA rule tags on each of the three pages, after each of the five sample decisions, after a keyboard slider change, with a pipeline step's trace opened, and with a field error: 88 scans. It fails on any violation, any horizontal overflow and any console error (which catches hydration mismatches and CSP blocks).
...
[ADR 0003](adr/0003-policy-tests-as-evals.md) explains why the golden expectations are hand-derived rather than snapshots, and how the same set powers the impact preview.
...
- **A versioned policy service with approvals.** Policies stored server-side as immutable, numbered versions. A decision records the policy version it ran under. No policy comes from the client, and the server's decision is the only one of record; any browser-side result is a preview.
